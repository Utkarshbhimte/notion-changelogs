...
const PostImage: React.FC<PostImageProps> = ({ post }) => {
  if (!post)
    return (
      <div className="h-screen w-screen flex items-center justify-center direction-column">
        <h1>Nothing here</h1>
      </div>
    )
  ...
  return (
    <div
      className="h-screen w-screen overflow-hidden bg-cover relative"
      style={{
        background: 'url(/gradient-background.jpg)',
        height: '100vh',
        width: '100vw',
        overflow: 'hidden',
      }}
    >
      ...
      <img
        style={{ width: '80vw', transform: 'translate(-50%, -20%)' }}
        className="rounded-3xl transform absolute left-1/2 top-1/2 shadow-xl"
        src={post?.thumbnail}
        alt=""
      />
    </div>
